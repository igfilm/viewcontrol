...
class TestDevices(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.process_cmd = processcmd.ThreadCmd(cls.logger, 
    ...
    def test_AtlonaATOMESW32(self):
        ...
        cmd_8 = show.CommandObject("Set Output Fail", "AtlonaATOMESW32", "Set Output", 1, 3)
        self.assertEqual(self.send_and_wait_for_answer(cmd_1).type, ComType.command_success)
        self.assertEqual(self.send_and_wait_for_answer(cmd_2).type, ComType.request_success)
        self.assertEqual(self.send_and_wait_for_answer(cmd_3).type, ComType.request_success)
        self.assertEqual(self.send_and_wait_for_answer(cmd_4).type, ComType.command_success)
        self.assertEqual(self.send_and_wait_for_answer(cmd_5).type, ComType.request_success)
        time.sleep(2)
        self.assertEqual(self.send_and_wait_for_answer(cmd_6).type, ComType.command_success)
        self.assertEqual(self.send_and_wait_for_answer(cmd_7).type, ComType.command_success)
        ...
        self.assertEqual(self.send_and_wait_for_answer(cmd_8).type, ComType.command_failed)
    ...
    def test_DenonDN500BD(self):
        ...
        cmd_4 = show.CommandObject("jump to start chapter", "DenonDN500BD", "Track Jump", 1)
